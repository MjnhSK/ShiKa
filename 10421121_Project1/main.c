...
//The function that merges images
...
{   
    ...
    for(int i=0; i < width * height * channel; i+=3){
        //fg[i] is Red value, fg[i+1] is Green value, fg[i+2] is Blue value
        if((fg[i] <= 180 && fg[i] >= 50) && (fg[i+1] <= 255 && fg[i+1] >= 150) && (fg[i+2] <= 100 && fg[i+2] >= 0))
        { 
            temp_array[i]=bg[i];
...
int main()
{
    ...
    // read image data
    //Background
    unsigned char *imagebg = stbi_load(path_imgbg, &width, &height, &channel, 3);
    if (imagebg == NULL)
    {
    ...
    //Foreground
    unsigned char *imagefg = stbi_load(path_imgfg, &width, &height, &channel, 3);
    if (imagefg == NULL)
    {
        printf("\nError in loading the image\n");
        exit(1);
